...
import { DialParams, mappingCurve, TrimRange } from './mapping-curve.util';
  ...
  template: `
    <svg [attr.width]="width()" [attr.height]="height()" [attr.viewBox]="'0 0 ' + width() + ' ' + height()">
      ...
      @for (band of curve().dead; track $index) {
        <rect [attr.x]="band.x" y="1" [attr.width]="band.width" [attr.height]="height() - 2" fill="#FFFFFF" opacity="0.04"></rect>
      }
      @if (curve().ghost) {
        <path [attr.d]="curve().ghost" fill="none" stroke="#FFB020" stroke-width="1" opacity="0.28"></path>
      }
...
export class MappingPreviewComponent {
  ...
  /** The control's trim range; without one the output is drawn across the full 0..100%. */
  ...
  readonly curve = computed(() => mappingCurve(this.dialParams(), this.geom(), this.responseCurve(), this.trim()));
}
