...
interface PlatformInfo { os: string; voicemeeter: boolean; waveLink: boolean; sonar: boolean; flatpak: boolean; autoUpdate: boolean; version: string; branch?: string | null; commit?: string | null; }
 ...
 * integrations (Voicemeeter and Sonar = Windows only; Wave Link = Windows/macOS only).
 */
...
export class PlatformService {
  ...
  readonly sonarSupported = computed(() => this.info.value()?.sonar ?? true);
