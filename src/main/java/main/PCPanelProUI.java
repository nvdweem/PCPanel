...
public class PCPanelProUI extends Device {
    ...
    @FXML
    private Pane sliderHolder1;

    @FXML
    private Pane sliderHolder2;

    @FXML
    private Pane sliderHolder3;

    @FXML
    private Pane sliderHolder4;

    @FXML
    private SVGPath sliderLabel1;

    @FXML
    private SVGPath sliderLabel2;

    @FXML
    private SVGPath sliderLabel3;

    @FXML
    private SVGPath sliderLabel4;

    @FXML
    private SVGPath logoLight;

    @FXML
    private SVGPath knobColor1;

    @FXML
    private SVGPath knobColor2;

    @FXML
    private SVGPath knobColor3;

    @FXML
    private SVGPath knobColor4;

    @FXML
    private SVGPath knobColor5;

    @FXML
    private Pane sliderLightPane1;

    @FXML
    private Pane sliderLightPane2;

    @FXML
    private Pane sliderLightPane3;

    @FXML
    private Pane sliderLightPane4;
    ...
    private static final double SLIDER_HEIGHT = 80.0D;
    ...
    public PCPanelProUI(String serialNum, DeviceSave deviceSave) {
        ...
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/assets/PCPanelPro/PCPanelPro.fxml"));
    ...
    private void rotateKnob(int knob, int val) {
        ...
        if (knob < 5) {
            ...
            double x = Util.map(val, 0.0D, 100.0D, sliderHolders[knob - 5].getPrefHeight(), 0.0D) - 40.0D;
    ...
    private void initButtons() throws IOException {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            knobs[i].setOnAction(e -> {
                ...
                String name = (knob < 5) ? ("Knob " + (knob + 1)) : ("Slider " + (knob - 5 + 1));
                String analogType = (knob < 5) ? "Knob" : "Slider";
                BasicMacro bm = new BasicMacro(this, knob, knob < 5, name, analogType);
                try {
                    childDialogStage = new Stage();
    ...
    private void setAllColor(Paint color) {
        byte b;
        int i;
        SVGPath[] arrayOfSVGPath;
        for (i = (arrayOfSVGPath = knobColors).length, b = 0; b < i; ) {
            SVGPath p = arrayOfSVGPath[b];
            p.setFill(color);
            b++;
        }
        for (i = (arrayOfSVGPath = sliderLabels).length, b = 0; b < i; ) {
            SVGPath p = arrayOfSVGPath[b];
            p.setFill(color);
            b++;
        }
        Pane[] arrayOfPane;
        for (i = (arrayOfPane = sliderLightPanes).length, b = 0; b < i; ) {
            Pane pane = arrayOfPane[b];
    ...
    public void showLightingConfigToUI(LightingConfig config) {
        ...
        if (mode == LightingMode.ALL_COLOR) {
            ...
            int row = 0;
            knobColor1.setFill(
                    Color.hsb((360 * (totalRows - row - 1) * (0xFF & config.getRainbowPhaseShift())) / 255.0D * totalRows, 1.0D, (0xFF & config.getRainbowBrightness()) / 255.0D));
            knobColor2.setFill(
                    Color.hsb((360 * (totalRows - row - 1) * (0xFF & config.getRainbowPhaseShift())) / 255.0D * totalRows, 1.0D, (0xFF & config.getRainbowBrightness()) / 255.0D));
            row++;
            knobColor3.setFill(
            ...
            int j;
            SVGPath[] arrayOfSVGPath;
            for (j = (arrayOfSVGPath = sliderLabels).length, b = 0; b < j; ) {
                SVGPath p = arrayOfSVGPath[b];
                p.setFill(Color.hsb((360 * (totalRows - row - 1) * (0xFF & config.getRainbowPhaseShift())) / 255.0D * totalRows, 1.0D, (0xFF & config.getRainbowBrightness()) / 255.0D));
                b++;
            }
            row++;
            for (int i = 4; i >= 0; i--) {
                for (int a = 0; a < 4; a++)
                    ((SVGPath) sliderLightPanes[a].getChildren().get(i)).setFill(
                            Color.hsb((360 * (totalRows - row - 1) * (0xFF & config.getRainbowPhaseShift())) / 255.0D * totalRows, 1.0D, (0xFF & config.getRainbowBrightness()) / 255.0D));
                row++;
            }
            logoLight.setFill(
            ...
            for (i = 0; i < 4; i++) {
                ...
                if (sliderConfig.getMode() == SINGLE_SLIDER_MODE.STATIC) {
                    for (Node n : sliderLightPanes[i].getChildren())
                        ((SVGPath) n).setFill(Paint.valueOf(sliderConfig.getColor1()));
                } else if (sliderConfig.getMode() == SINGLE_SLIDER_MODE.STATIC_GRADIENT) {
                    Color c1 = Color.web(sliderConfig.getColor1());
                    Color c2 = Color.web(sliderConfig.getColor2());
                    double f = 0.0D, delta = 0.25D;
                    for (int a = 0; a < 5; a++) {
