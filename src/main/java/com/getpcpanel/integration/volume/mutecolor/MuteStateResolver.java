...
public interface MuteStateResolver {
    /** Sentinel target meaning "follow whatever this control's own commands act on". */
    String FOLLOW = "__follow__";

    /**
     * @param command the control's commands: its turn (dial/slider) actions merged with its press
     *                (button) actions, so a control muted by a mute button resolves just like one
     *                muted through its volume dial
