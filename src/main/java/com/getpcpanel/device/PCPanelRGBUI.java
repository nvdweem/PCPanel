...
public class PCPanelRGBUI extends Device {
    ...
    public void showLightingConfigToUI(LightingConfig config) {
        ...
        if (mode == LightingMode.ALL_COLOR) {
            ...
            startLightingAnimation("RGB_ALL_RAINBOW", elapsedNs -> applyRainbow(config, elapsedNs));
        } else if (mode == LightingMode.ALL_WAVE) {
            startLightingAnimation("RGB_ALL_WAVE", elapsedNs -> applyWave(config, elapsedNs));
        } else if (mode == LightingMode.ALL_BREATH) {
            startLightingAnimation("RGB_ALL_BREATH", elapsedNs -> applyBreath(config, elapsedNs));
        }
    }
