...
public final class InputInterpreter {
    ...
    public void onButtonPress(@Observes ButtonPressEvent event) throws IOException {
    ...
    private void doDialAction(String serialNum, boolean initial, int knob, DialValue v) {
        save.getProfile(serialNum).map(p -> p.getDialData(knob)).ifPresent(data -> eventPublisher.fire(new PCPanelControlEvent(serialNum, knob, data, initial, v)));
    }
    ...
    private void determineClick(ClickId clickId, long timeDiff) {
        ...
        if (isDblClick) {
            ...
            eventPublisher.fire(new ButtonClickEvent(clickId.serialNum(), clickId.button(), true));
            lastClicks.remove(clickId);
            return;
        }

        lastClicks.put(clickId, System.currentTimeMillis());
        Runnable trigger = () -> eventPublisher.fire(new ButtonClickEvent(clickId.serialNum(), clickId.button(), false));
    ...
    public void onButtonPress(@Observes ButtonClickEvent event) {
        save.getProfile(event.serialNum()).ifPresent(profile -> {
            ...
            if (event.dblClick() && hasCommands(dblClick)) {
                eventPublisher.fire(new PCPanelControlEvent(event.serialNum(), event.button(), dblClick, false, null));
            } else if (!event.dblClick() && hasCommands(click)) {
                eventPublisher.fire(new PCPanelControlEvent(event.serialNum(), event.button(), click, false, null));
            }
        });
    }
